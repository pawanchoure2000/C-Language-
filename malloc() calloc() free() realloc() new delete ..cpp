...
int main()
{
	unsigned char buf[sizeof(int)*2];		//buffer  on stack, global area 
	...
	int *pbuf=(int*) (buf +0) ;				int *qbuf=(int*)(buf + sizeof(int));		//address are not coming from dynamic store but from dynamic store that we have provided  
...
/*
//alloctar         deallocatar 
malloc()           free()
operator new  		opertar delete 
operator  new[]     opertar delete[]
opertar new ()      no delete 
*/


//overloading opertar new 
void *operator new(size_t n)	//size_t is unsigned int 
{
	cout<<"overloade new ";
	void *ptr;
	ptr=malloc(n);
